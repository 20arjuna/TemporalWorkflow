#!/usr/bin/env python3
"""
Test script to verify order activities integration with database.
Tests activities directly without Temporal worker/client.
"""
...
async def test_order_activities_db():
    """Test order activities with database integration."""
    print("🧪 Testing Order Activities with Database Integration")
    print("=" * 60)
    
    # Test data
    test_order_id = f"TEST-ACTIVITIES-{int(datetime.now().timestamp())}"
    test_address = {
        "line1": "789 Activities Test Blvd",
        "city": "Test City",
        "state": "TC", 
        "zip": "98765"
    }
    ...
    try:
        ...
        # Import activities here to avoid Temporal import issues
        from activities.order_activities import receive_order, validate_order, charge_payment
        
        # Test each activity
        await test_receive_order_activity(receive_order, test_order_id, test_address)
        await test_validate_order_activity(validate_order, test_order_id, test_address)
        await test_charge_payment_activity(charge_payment, test_order_id, test_address)
        
        # Verify database state
        await verify_database_state(test_order_id)
        
        # Test idempotency
        await test_payment_idempotency(charge_payment, test_order_id, test_address)
        
        print("\n🎉 ALL TESTS PASSED! Order activities are working with database! 🚀")
        return True
...
async def test_receive_order_activity(receive_order_func, order_id: str, address: dict):
    """Test the receive_order activity."""
    print(f"\n📦 Testing receive_order activity...")
    
    order_data = {"order_id": order_id, "address": address}
    result = await receive_order_func(order_data)
...
async def test_validate_order_activity(validate_order_func, order_id: str, address: dict):
    """Test the validate_order activity."""
    print(f"\n✅ Testing validate_order activity...")
    
    order_data = {"order_id": order_id, "address": address}
    result = await validate_order_func(order_data)
...
async def test_charge_payment_activity(charge_payment_func, order_id: str, address: dict):
    ...
    try:
        result = await charge_payment_func(payment_data)
...
async def test_payment_idempotency(charge_payment_func, order_id: str, address: dict):
...
async def main():
    """Run the order activities database integration test."""
    success = await test_order_activities_db()
    return success
